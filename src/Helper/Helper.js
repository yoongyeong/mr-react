...
const handlePostingDate = str => {
	...
	if (str > now - oneMinute) {
		...
		return "Yesterday at " + str.getHours() + ":" + minuteTwoDigits + str.getMinutes();
	} else {
		return date.format(str);
	}
};

const handleBirthDate = str => {
...
function getAge(birthDate) {
	...
	let date1_UTC = new Date(Date.UTC(birthDate.getUTCFullYear(), birthDate.getUTCMonth(), birthDate.getUTCDate()));
...
export { handlePostingDate, handleBirthDate, getAge };
